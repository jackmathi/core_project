...
$link = mysql_connect($server, $user, $pass) or die("Couldn't make connection.");
...
define ("USER_LEVEL", 1);
define ("EMPLOYEE_LEVEL", 2);
define ("COTELECALLING_LEVEL", 3);
...
setcookie("my_id", '', time()-60*60*24*COOKIE_TIME_OUT, "/");
...
session_start();
if(isset($_SESSION['user_id']) || isset($_COOKIE['user_id'])) {
mysql_query("update `register_details` 
			set `ckey`= '', `ctime`= '' 
			where `id`='$_SESSION[user_id]' OR  `id` = '$_COOKIE[user_id]'") or die(mysql_error());
}			
...
setcookie("my_id", '', time()-60*60*24*COOKIE_TIME_OUT, "/");
setcookie("user_name", '', time()-60*60*24*COOKIE_TIME_OUT, "/");
setcookie("firstname", '', time()-60*60*24*COOKIE_TIME_OUT, "/");
setcookie("user_key", '', time()-60*60*24*COOKIE_TIME_OUT, "/");
...
function checkCotelecalling() {

if($_SESSION['user_level'] == COTELECALLING_LEVEL) {
return 1;
} else { return 0 ;
}
}
